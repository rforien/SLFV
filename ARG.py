...
class GenomePartition(object):
    ...
    def partition(self, locus):
        ...
        index = np.argsort(segments_at_locus['individual'].values)
        partition = segments_at_locus['lineage'][index]
        return partition
    ...
    def merge_lineages(self, lineages_to_merge, newlabels = None,
        ...
        # update segments
        segments = pd.concat((self.segments.loc[~np.isin(self.segments['lineage'], lineages_to_merge)],
        ...
        if self.labels is not None:
            ...
            labels = None
        if inplace:
            self.__init__(self.G, self.n, segments, labels)
            if return_IBD_segments:
                return self.IBD_segments
        else:
            new_GP = GenomePartition(self.G, self.n, segments, labels)
            if return_IBD_segments:
    ...
    def drop_lineages(self, min_segment_length, inplace = True):
        if inplace:
            self.segments = self.segments.loc[self.segments['end'] - self.segments['start'] >= min_segment_length]
            return self
        else:
...
if __name__ == '__main__':
    ...
    GP, segments = GP.merge_lineages([0, 1], return_IBD_segments=True, min_segment_length=0.01)
